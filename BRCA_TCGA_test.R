...
#minGSSize和maxGSSize最小和最大尺寸的注释
#organism= 物种
#save(gene_up,gene_down,gene_all,file='enrichKEGG.Rdata')
kk.down <- enrichKEGG(gene = gene_down,
                      organism = 'human',
                      pvalueCutoff = 0.5,
                      universe = gene_all)
head(kk.up)[1:6]
kk.up.frame <- as.data.frame(kk.up)
kk.down.frame <- as.data.frame(kk.down)
...
go.down <- enrichGO(gene = gene_down,
                   ...
                   qvalueCutoff = 0.01,
                   readable = TRUE)
go.down.frame <- as.data.frame(go.down)
head(go.down.frame)[,1:6]
#minGSSize： 每个基因集的最小数目
#maxGSSize：每个基因集最大数目
#有点设置min=10，max=500
#clusterprofiler支持DAVID、GO、KEGG、gseGO、gseKEGG、merge_result和简单画图。
...
##########################
library("ggplot2")
down_kegg <- kk.down.frame[kk.down.frame$pvalue<0.05,]
down_kegg$group = -1
up_kegg <- kk.up.frame[kk.up.frame$pvalue<0.05,]
up_kegg$group <- 1
...
g_kegg <- ggplot(dat,aes(x=reorder(Description,order(pvalue,decreasing = F)),y=pvalue,fill=group)) +
  ...
  scale_y_continuous(name = "log10P_value")+
...
data <- data.frame(g=group_list,
                   p=colnames(exprSet),
                   v= as.numeric(exprSet[rownames(DESeq2_DEG[3,])])
                   )

ggpaired(data,x = 'g',y = 'v',
